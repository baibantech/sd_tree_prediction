		...
		if (pcur->pos + 1 < first_chbit)
			...
			if (cur_data >= 0 && cur_data < SPT_DB_MODULE_INVALID) {
				pdh_ext = (struct spt_module_dh_ext *)db_module_id_2_ptr(pclst,
						cur_data);
				pcur_data = pdh_ext->data;
				cmp = diff_identify(pdata, pcur_data, 0, pcur->pos + 1, &cmpres);
					
				pqinfo->db_id = cur_data;
				pqinfo->vec_id = vec_id_stack[i - 1];
				if (cmp == 0) {
					printf("first chbit is %d, pcur is %p\r\n", first_chbit, pcur);
					spt_assert(0);
				} else if (cmp > 0)
					pqinfo->cmp_result = 1;
				else
...
int module_tree_hash_node;
int module_tree_hash_branch;
int module_tree_hash_preseg_ok;
int module_tree_hash_preseg_err;
		...
		unsigned int *window_hash, unsigned int *seg_hash)
{
	...
	grama_one_byte = 1;
	spt_trace("cur len is %d, window len is %d\r\n",cur_pos_len, window_len);
	if (cur_pos_len != window_len)
		*window_hash = djb_hash_seg(window_byte - 1, grama_seg_hash, cur_pos_len - window_len);
	else
		*window_hash = grama_seg_hash;
	spt_trace("window hash is %x\r\n", *window_hash);
	if (pos % 8)
		*window_hash = *window_hash + (*cur_byte >> (8 - bit_offset));
	...
	while (((unsigned long)(void*)window_byte) <= ((unsigned long) (void*)cur_byte)){
		...
		if (ret >= 0) {
			...
			if (vec.pos >= bit_offset) {
				module_tree_hash_branch++;
				spt_trace("data pre seg hash is %x\r\n", pre_grama_seg_hash);
				if (grama_one_byte) {
					vecid = vec_judge_preseg_full_and_alloc(vec_clst, ret_vec, pre_grama_seg_hash);
					if (vecid != -1) {
						if ((cnt == 1) && (pos > 192 *8))
							module_tree_hash_one_byte++;
						spt_trace("alloc vec %p\r\n", *ret_vec);
						module_tree_alloc_ok++;
						module_tree_hash_preseg_ok++;
						*seg_hash = pre_grama_seg_hash;
						return vecid;
					} else {
						vecid = vec_alloc(vec_clst, ret_vec, pre_grama_seg_hash);
						*seg_hash = pre_grama_seg_hash;
						module_tree_hash_preseg_err++;
						return vecid;
					}
				}
			}
			else
				module_tree_hash_node++;
#endif
...
unsigned int find_start_vec_err1;
unsigned int find_start_vec_err2;
